...
import { useAceitarRecomendacao } from "@/hooks/useRecomendacoes"
...
export default function RecommendationCard({ recomendacao, isBest, onVerDetalhes }: RecommendationCardProps) {
  const { id, servico, probabilidade_adequacao, explicacao } = recomendacao
  const [showFeedback, setShowFeedback] = useState(false)
  const { mutate: aceitar, isPending } = useAceitarRecomendacao()
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col gap-1">
          ...
          <h3 className="font-heading text-2xl font-bold text-primary break-words">
            {servico.nome}
          </h3>
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            <div
              className={`h-full rounded-full ${adequacaoColor} transition-all duration-500`}
