...
import { Brain, Loader2, CheckCircle2, Circle, TrendingUp, Award } from "lucide-react";
...
export default function HistoricoModelosPage() {
  ...
  return (
    <div className="flex flex-col gap-8 p-6 max-w-7xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Navegação por Abas */}
      <div className="flex gap-2 p-1 rounded-xl bg-white/5 border border-white/5 self-start shrink-0">
        <button
      ...
      {activeTab === "resultados" && (
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          {/* Card explicativo */}
          <div className="rounded-xl glass-card border-white/5 p-6 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              ...
              <div key={metric.label} className="rounded-xl glass-card p-5 border-white/5 flex flex-col gap-3 relative hover:border-white/10 transition-all">
                <span className="text-xs text-muted-foreground font-semibold min-h-[32px]">{metric.label}</span>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl font-extrabold text-white">{(metric.val * 100).toFixed(1)}%</span>
                ...
                <p className="text-[11px] text-muted-foreground/75 leading-normal mt-1">{metric.desc}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Matriz de Confusão */}
            <div className="rounded-xl glass-card p-6 border-white/5 flex flex-col gap-4">
              <div>
