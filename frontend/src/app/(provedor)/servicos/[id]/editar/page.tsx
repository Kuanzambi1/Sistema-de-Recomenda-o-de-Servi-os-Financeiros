...
export default function EditarServicoPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const servico = await servicosService.obter(id);
        reset({
          nome: servico.nome,
          tipo: servico.tipo,
          descricao: servico.descricao,
          taxa_juro_anual: servico.taxa_juro_anual,
          montante_minimo: servico.montante_minimo,
          montante_maximo: servico.montante_maximo,
          prazo_minimo_meses: servico.prazo_minimo_meses,
          prazo_maximo_meses: servico.prazo_maximo_meses,
          rendimento_minimo: servico.rendimento_minimo,
          score_credito_minimo: servico.score_credito_minimo,
          requer_conta_bancaria: String(servico.requer_conta_bancaria),
        });
      } catch (err: any) {
  ...
  const onSubmit = async (data: any) => {
    ...
    setError(null);
    try {
      await servicosService.actualizar(id, {
        nome: data.nome,
      ...
      setError(err?.message ?? "Erro ao actualizar o serviço. Verifique os dados.");
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-4xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-sm">Actualize as condições do seu produto financeiro.</p>
      </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <select id="tipo" required {...register("tipo")} className="h-8 w-full min-w-0 rounded-lg border border-border bg-muted/30 px-2.5 py-1 text-sm transition-colors outline-none focus-visible:border-primary/50 focus-visible:ring-3 focus-visible:ring-primary/20 md:text-sm">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            <div className="flex flex-col gap-2">
              ...
              <Input id="rend_min" type="number" min="0" required {...register("rendimento_minimo")} className="bg-muted/30 border-border focus:border-primary/50" />
            </div>
            <div className="flex flex-col gap-2">
              <Label htmlFor="score_min">Score Crédito Mínimo (0-1000) *</Label>
              <Input id="score_min" type="number" min="0" max="1000" required {...register("score_credito_minimo")} className="bg-muted/30 border-border focus:border-primary/50" />
            </div>
            <div className="flex flex-col gap-2">
              <Label htmlFor="conta">Requer Conta Bancária?</Label>
              <select id="conta" {...register("requer_conta_bancaria")} className="h-8 w-full min-w-0 rounded-lg border border-border bg-muted/30 px-2.5 py-1 text-sm transition-colors outline-none focus-visible:border-primary/50 focus-visible:ring-3 focus-visible:ring-primary/20 md:text-sm">
